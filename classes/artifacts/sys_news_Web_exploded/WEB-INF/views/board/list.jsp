	...
	<div class="layui-col-md12">
		<div class="layui-card">
			...
			<div class="layui-card-body">
			     <form action="BoardServlet?action=BoardList" method="post">
						    
						         <div class="layui-form-item">
									    <div class="layui-inline">
									      <div class="layui-input-inline">
									       		<input type="text" placeholder="根据公告标题" name="names" id ="names" class="layui-input"  />
									      </div>
									    </div>
									    <div class="layui-inline">
						...
						<c:forEach items="${list}" var="u">
							<tr>
							    ...
							    <input id ="${u.id}" value="${u.id}" type="hidden" class="weui-input"/>
										<input id ="${u.id}" value="${u.id}" type="hidden" class="weui-input"/>
										<a href="BoardServlet?action=toUpdateBoard&id=${u.id}"
				...
				<div style="text-align: center">
					<div class="layui-btn-group">
						...
						<a class="layui-btn  layui-btn-primary"
							href="BoardServlet?action=BoardList&p=${tp}">尾页</a> 
					</div>
				</div>
...
layui.use([ 'form','jquery','layer','laydate' ], function() {
	var form = layui.form,
	 ...
	 $('a.agree').click(function(e){
	   		            var id = $(e.currentTarget).parent().find('.weui-input').attr('id');
				   		if(confirm("确定要同意吗？")){
				   		  $.post("BoardServlet?action=updateStatus&id="+id+"&state=1", function(data) {
	 ...
	 $('a.refused').click(function(e){
	   		            var id = $(e.currentTarget).parent().find('.weui-input').attr('id');
				   		if(confirm("确定要拒绝吗？")){
					   		  $.post("BoardServlet?action=updateStatus&id="+id+"&state=2", function(data) {
				   			   layer.msg('操作成功！',{icon:6,offset:"auto",time:2000},function(){
							    	document.location.reload();//当前页面
							    });
				   		   }); 
